...
test = pd.read_csv(path + 'jinnan_round1_test_20190121.csv', engine='python')
#testC = 
...
#numerical_features = ['样本id', 'diffTarget']
#categorical_features = [f for f in data.columns if f not in numerical_features]
categorical_features = [f for f in data.columns if f not in ['样本id', 'diffTarget']]
categorical_features = [f for f in categorical_features if f not in addFeatures]
numerical_features = [f for f in data.columns if f not in categorical_features]
...
def GetMeanFeature(train, test, meanFeatures):
    train['target'] = list(newTarget)
    train['intTarget'] = pd.cut(train['target'], 5, labels=False)
    train = pd.get_dummies(train, columns=['intTarget'])
    temp = train.columns[-5:].tolist()
    for feat1 in categorical_features:
        cate_rate = train[feat1].value_counts(normalize=True, dropna=False).values[0]
        if cate_rate < 0.70:
            for feat2 in temp:
                col_name = 'B14_to_'+feat1+"_"+feat2+'_mean'
                meanFeatures.append(col_name)
                order_label = train.groupby([feat1])[feat2].mean()
                train[col_name] = train['B14'].map(order_label)
                miss_rate = train[col_name].isnull().sum() * 100 / train[col_name].shape[0]
                if miss_rate > 0:
                    train = train.drop([col_name], axis=1)
                    meanFeatures.remove(col_name)
                else:
                    test[col_name] = test['B14'].map(order_label)    
    train.drop(temp+['target'], axis=1, inplace=True)
    return train, test, meanFeatures

meanFeatures = []
train, test, meanFeatures = GetMeanFeature(train, test, meanFeatures)

print(train.shape)
print(test.shape)
print('--------------')

X_train = train[meanFeatures+numerical_features].values  # 数值类型数据不需要编码
X_test = test[meanFeatures+numerical_features].values
...
for feat in categorical_features:
    ...
    X_train = sparse.hstack((X_train, OHE.transform(train[feat].values.reshape(-1, 1))), 'csr')  # 添加离散型数据
    X_test = sparse.hstack((X_test, OHE.transform(test[feat].values.reshape(-1, 1))), 'csr')
...
submission.to_csv('./submit_C.csv', index=False, header=None)
print('done!')
