...
def unbalanced_sample():
    """不平衡样本优化，把训练集0样本分成6、8、12份分别与1样本合成新的训练集，然后再取平均值求和
    """
    ...
    # 分离0样本和1样本
    train_dataset_label_0 = train_dataset.query('label == 0')
    train_dataset_label_1 = train_dataset.query('label == 1')
    
    # 特征和标签分离
    X_0 = train_dataset_label_0.drop(['rrname', 'label'], axis=1).values
    y_0 = train_dataset_label_0['label'].values
    X_1 = train_dataset_label_1.drop(['rrname', 'label'], axis=1).values
    y_1 = train_dataset_label_1['label'].values
    
    # 生成随机列表
    index = [i for i in range(train_dataset_label_0.shape[0])]
    random.shuffle(index)

    # 均分成8份，138144/8=17268
    loc = 17268
    for i in range(8):
        idx = index[loc*i:loc*(i+1)]
        X = X_0[idx]
        y = y_0[idx]
        ...
        save_training_model(model, score, model_path, score_path)
