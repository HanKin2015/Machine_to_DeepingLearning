...
class PEFile:
    def __init__(self, root, file):
        ...
        self.HasDebug             = pe.has_debug                # 是否包含debug信息
        self.NumbersOfExFunctions = len(pe.exported_functions)  # 导出函数的个数
        self.NumbersOfImFunctions = len(pe.imported_functions)  # 导入函数的个数
        self.HasRelocations       = pe.has_relocations          # 是否启用了重定向
        self.HasResources         = pe.has_resources            # 是否包含资源文件
        self.HasSignature         = pe.has_signature            # 是否包含信号量
        self.HasTls               = pe.has_tls                  # 是否启用了TLS回调函数
        self.NumberOfSymbols      = len(pe.symbols)             # 符号个数
...
def get_pefile_info(root, file):
    ...
    try:
        ...
        print('file[{}] get info failed, error[{}].'.format(file, e))
...
def get_dataset(data_path):
    ...
    for task in tasks:
        ...
        if sample is not None:
            ...
            invalid_sample += 1
    print('{} has {} invalid sample.'.format(data_path, invalid_sample))
    return dataset
